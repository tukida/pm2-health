...
export class Health {
  ...
  constructor(private _config: IConfig) {
    ...
    if (
      this._config.metricIntervalS == null ||
  ...
  async fetchConfig() {
    ...
    try {
      ...
      if (this._config.webConfig.auth && this._config.webConfig.auth.user)
        // auth
        fetch.basicAuth(
          this._config.webConfig.auth.user,
          this._config.webConfig.auth.password
        );
  ...
  configChanged() {
    ...
    if (Array.isArray(this._config.messageExcludeExps))
      this._messageExcludeExps = this._config.messageExcludeExps.map(
        (e) => new RegExp(e)
      );
  ...
  isAppIncluded(app: string) {
    if (app === 'pm2-health-custom') return false;
  ...
  async go() {
    ...
    PM2.connect((ex) => {
      ...
      PM2.launchBus((ex, bus) => {
        ...
        bus.on('process:event', (data) => {
          ...
          if (
            Array.isArray(this._config.events) &&
            this._config.events.indexOf(data.event) === -1
          )
            return;
          ...
          this._notify.send({
            ...
            priority: 'high',
            attachements: LOGS.filter(
              (e) => this._config.addLogs === true && data.process[e]
            ).map((e) => ({
              filename: basename(data.process[e]),
              path: data.process[e]
            }))
          });
        });
    ...
    Pmx.action('debug', undefined, (reply) => {
      PM2.list((ex, list) => {
        ...
        Fs.writeFileSync('pm2-health-custom-debug.json', JSON.stringify(list));
        Fs.writeFileSync(
          'pm2-health-custom-config.json',
      ...
      for (const app of list) {
        ...
        if (app.pm2_env) {
          ...
          if (app.pm2_env['node_version'])
            monit['node'] = {
              value: app.pm2_env['node_version'],
              direct: true
            };
        }
        ...
        for (const key of Object.keys(monit)) {
          ...
          if (!probe.direct) {
            ...
            if (Number.isNaN(v)) {
              error(
                `monit [${app.name}.${key}] -> [${monit[key].value}] is not a number`
              );
              continue;
            }
          }
          ...
          if (
            probe.noNotify !== true &&
            bad === true &&
            (probe.ifChanged !== true ||
              this._snapshot.last(app.pm_id, key) !== v)
          )
            alerts.push(
              `<tr><td>${app.name}:${
                app.pm_id
